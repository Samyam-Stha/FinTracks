...
const AnalyticsPage = ({ setCurrentPage }) => {
  ...
  const [savingsBudgetData, setSavingsBudgetData] = useState(null);
  // Add state for selected month/year and loading for savings filter
  const [savingsMonth, setSavingsMonth] = useState(new Date().getMonth() + 1);
  const [savingsYear, setSavingsYear] = useState(new Date().getFullYear());
  const [savingsLoading, setSavingsLoading] = useState(false);
  // Add a new state for filtered savings budget data
  const [filteredSavingsBudgetData, setFilteredSavingsBudgetData] = useState(null);
  ...
  // Ensure graphsData is fetched when trends tab is first loaded
  useEffect(() => {
    if (activeTab === 'trends' && !graphsData) {
      fetchGraphsData();
    }
  }, [activeTab]);

  // Ensure data is fetched on component mount if trends tab is active
  useEffect(() => {
    if (activeTab === 'trends') {
      fetchGraphsData();
    }
  ...
  // Accept summary as a parameter
  const getCurrentPeriodLabel = (summary) => {
    const now = new Date();
    switch (interval) {
      case 'daily':
      ...
      case 'weekly': {
        ...
        let weekNumber = Math.ceil((dayOfMonth + firstDay.getDay()) / 7);
        const weekLabels = summary?.data
          ? summary.data.map(e => e.label).filter(label => /^Week \d+$/.test(label))
          : [];
        const maxWeek = weekLabels.length > 0 ? weekLabels.length : 4;
        weekNumber = Math.min(weekNumber, maxWeek);
            ...
            <CardContent>
              <div className="h-80 transition-all duration-500 ease-in-out">
                {interval === 'weekly' && summary?.data && (
                  <>
                    {console.log('Weekly chart labels:', summary.data.map(e => e.label))}
                    {console.log('Current period label:', getCurrentPeriodLabel(summary))}
                  </>
                )}
  ...
  const renderTrendsTab = () => {
    ...
    // Prepare data for budget vs actual bar chart
    const budgetChartData = budgetData?.comparison?.map(item => ({
      category: item.category,
      budget: item.budget,
      actual: item.actual,
      variance: item.variance
    ...
    // Prepare data for expense category pie chart
    const expenseChartData = graphsData?.expenses?.map((item, index) => ({
      name: item.category,
      value: Number(item.total),
      color: CHART_COLORS[index % CHART_COLORS.length]
    })) || [];
              ...
              <Select value={tempMonth.toString()} onValueChange={v => setTempMonth(Number(v))}>
                ...
                <SelectContent>
                  {Array.from({ length: 12 }, (_, i) => i + 1)
                    .filter(month => {
                      const now = new Date();
                      if (Number(tempYear) === now.getFullYear()) {
                        // Only allow months strictly less than the current month
  ...
  const renderSavingsTab = () => {
    // Debug: log the API data for the selected month/year
    console.log('filteredSavingsBudgetData', JSON.stringify(filteredSavingsBudgetData, null, 2));
    console.log('filteredSavingsBudgetData.comparison', filteredSavingsBudgetData?.comparison);
    // Use comparison if available, otherwise fallback to currentCategories only for current month
    const isCurrentMonth =
      savingsYear === new Date().getFullYear() &&
      savingsMonth === new Date().getMonth() + 1;

    const categorySource =
      (filteredSavingsBudgetData?.comparison && filteredSavingsBudgetData.comparison.length > 0)
        ? filteredSavingsBudgetData.comparison
        : (isCurrentMonth ? (filteredSavingsBudgetData?.currentCategories || []) : []);

    const categoryContributions = categorySource
      .filter(item => {
        // For comparison: item.variance > 0
        // For currentCategories: item.budget - item.spent > 0
        if ('variance' in item) return item.variance > 0;
        return (item.budget - item.spent) > 0;
      })
      .map(item => {
        if ('variance' in item) {
          return {
            ...
            contributionToSavings: item.variance,
            contributionPercentage: savingsData?.currentSavings > 0
              ? (item.variance / savingsData.currentSavings) * 100
              : 0
          };
        } else {
          // For currentCategories
          ...
          return {
            ...
            actual: item.spent,
            variance,
            variancePercentage: item.budget > 0 ? (variance / item.budget) * 100 : 0,
            contributionToSavings: variance,
            contributionPercentage: savingsData?.currentSavings > 0
              ? (variance / savingsData.currentSavings) * 100
              : 0
          };
        }
      })
    ...
    const chartData = categoryContributions.map((item, index) => ({
      name: item.category,
      value: item.variance,
    ...
    const totalSavingsFromCategories = categoryContributions.reduce((sum, item) => sum + item.variance, 0);
    const otherSavings = (savingsData?.currentSavings || 0) - totalSavingsFromCategories;

    // Find the selected month's saving entry and calculate percentage
    const selectedMonthSaving = savingsData?.monthlySavings?.find(
      s => new Date(s.month).getFullYear() === savingsYear && new Date(s.month).getMonth() + 1 === savingsMonth
    );
    const savingsPercentage = selectedMonthSaving && selectedMonthSaving.savingGoal > 0
      ? Math.round((selectedMonthSaving.savedAmount / selectedMonthSaving.savingGoal) * 100)
      : 0;
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-2 items-center mb-4">
          <select value={savingsYear} onChange={e => setSavingsYear(Number(e.target.value))} className="border rounded px-2 py-1">
            {getAvailableYears().map(year => (
              <option key={year} value={year}>{year}</option>
            ))}
          </select>
          <select value={savingsMonth} onChange={e => setSavingsMonth(Number(e.target.value))} className="border rounded px-2 py-1">
            {Array.from({ length: 12 }, (_, i) => i + 1).map(month => (
              <option key={month} value={month}>{getMonthName(month)}</option>
            ))}
          </select>
          {savingsLoading && (
            <span className="ml-2 animate-spin inline-block w-5 h-5 border-2 border-blue-500 border-t-transparent rounded-full"></span>
          )}
        </div>
        {/* Summary Cards */}
        ...
        <CategoryContributionToSavings
          chartData={chartData}
          totalSavingsFromCategories={totalSavingsFromCategories}
          otherSavings={otherSavings}
          savingsData={savingsData}
          categoryContributions={categoryContributions}
          CHART_COLORS={CHART_COLORS}
          selectedMonthSaving={selectedMonthSaving}
  ...
  // Memoize chart data and derived values
  const memoizedSummaryData = useMemo(() => graphsData?.summary?.data || [], [graphsData]);
  const memoizedTrendsData = useMemo(() => trendsData?.trends || [], [trendsData]);
  const memoizedBudgetData = useMemo(() => budgetData?.comparison || [], [budgetData]);
  const memoizedExpenseData = useMemo(() => graphsData?.expenses || [], [graphsData]);

  // Memoize event handlers
  const handleTabChange = useCallback((tabId) => setActiveTab(tabId), []);
  const handleIntervalChange = useCallback((intValue) => setInterval(intValue), []);

  // Function to fetch budget-vs-actual for selected month/year
  const fetchSavingsBudgetData = async (year, month) => {
  ...
  useEffect(() => {
    if (activeTab === 'savings') {
      fetchSavingsBudgetData(savingsYear, savingsMonth);
    }
  }, [savingsYear, savingsMonth, activeTab]);

  // Update the fetch function for filtered data
  const fetchFilteredSavingsBudgetData = async (year, month) => {
    setSavingsLoading(true);
    try {
      const res = await axios.get(
        `http://localhost:5000/api/analytics/budget-vs-actual?period=past&year=${year}&month=${month}`,
  ...
  useEffect(() => {
    if (activeTab === 'savings') {
      fetchFilteredSavingsBudgetData(savingsYear, savingsMonth);
    }
  }, [savingsYear, savingsMonth, activeTab]);
